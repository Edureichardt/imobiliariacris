...
export default function EditarImoveis() {
  ...
  const irParaEditar = (id: string) => {
    console.log('id recebido na navegação:', id);

    router.push(`/admin/editar-imoveis/${id}`);
  };

  return (
    <div className="p-6 max-w-7xl mx-auto">
      <h1 className="text-2xl font-bold mb-4">Editar Imóveis</h1>
      ...
      {imoveisFiltrados.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {imoveisFiltrados.map(imovel => {
            console.log('imovel.id:', imovel.id);
            return (
              <div key={imovel.id} className="border rounded shadow p-4">
                <Image
                  src={imovel.fotos?.[0] || '/placeholder.png'}
                  alt={imovel.tipo}
                  width={400}
                  height={300}
                ...
                <button
                  onClick={() => irParaEditar(imovel.id)}
                  className="w-full py-2 px-4 rounded bg-blue-600 hover:bg-blue-700 text-white transition-colors duration-200"
                >
                  Editar
                </button>
              </div>
